...
export default class Game extends React.Component {
  ...
  componentDidMount() {
    ...
    socket.on("disconnectButton", () => {
      alert("Opponent left");
      this.state.socket.emit("leaveGame", {
    ...
    socket.on("drawButton", () => {
      if (window.confirm("Draw?")) {
        ...
        this.state.socket.emit("gameResult", {
          gameId: this.state.gameId,
          userId: this.state.userId,
          result: "Draw",
        });
  ...
  resignButton = () => {
    if (this.state.disabled && this.state.turn === "white") {
      this.setState({
        disabled: true,
        status: "White Won",
        hideButton: "",
  ...
  drawButton = () => {
    ...
    this.state.socket.emit("askDraw", {
      userId: this.state.userId,
      gameId: this.state.gameId,
    });
  };
    ...
    if (this.state.sourceSelection === -1) {
      ...
      if (!squares[i] || squares[i].player !== this.state.player) {
        ...
        if (i === 4 || i === 60) {
          if (this.state.turn === "white" && this.state.whiteKingFirstMove) {
            if (
              ...
              squares[58] === null &&
              squares[59] === null &&
              !this.state.allPossibleMovesBlack.some((element) =>
            ...
            if (
              ...
              squares[2] === null &&
              squares[3] === null &&
              !this.state.allPossibleMovesWhite.some((element) =>
        ...
        if (squares[i].name === "Pawn") {
          ...
          highLightMoves = this.checkMovesVer2(
            squares,
            squares[i].possibleMoves(
              i,
              squares,
              enpassant,
              this.state.lastTurnPawnPosition
            ),
            i,
            this.state.turn
          );
      ...
      if (squares[this.state.sourceSelection].name === "Pawn") {
        ...
        if (this.state.highLightMoves.includes(i)) {
          ...
          if (
            ...
            if (squares[this.state.sourceSelection].name === "Pawn") {
              if (
                squares[this.state.sourceSelection].player === 1 &&
                i === this.state.sourceSelection - 16
              ) {
                firstMove = true;
              } else if (
                squares[this.state.sourceSelection].player === 2 &&
                i === this.state.sourceSelection + 16
              ) {
                firstMove = true;
              }
            ...
            //to convert pawn that reach other side of the chess board
            if (
              [0, 1, 2, 3, 4, 5, 6, 7, 56, 57, 58, 59, 60, 61, 62, 63].includes(
                i
              )
            ) {
        ...
        //for castling
        if (
          this.state.highLightMoves.includes(i) &&
          (i === 2 || i === 6 || i === 58 || i === 62) &&
          (this.state.whiteKingFirstMove || this.state.blackKingFirstMove)
        ) {
          if (i === 58) {
            squares = this.movePiece(i, squares, this.state.sourceSelection);
              ...
              if (squares[i].name === "Bishop") {
                if (
                  [
                    ...
                    33, 35, 37, 39, 40, 42, 44, 46, 49, 51, 53, 55, 56, 58, 60,
                    62,
                  ].includes(i)
                ) {
  ...
  enpassant(selectedPawnPosition) {
    let enpassant = false;
    if (
      selectedPawnPosition - 1 === this.state.lastTurnPawnPosition ||
      selectedPawnPosition + 1 === this.state.lastTurnPawnPosition
    ) {
  ...
  checkMovesVer2(squares, highLightMoves, i, turn) {
    ...
    for (let i = 0; i < highLightMoves.length; i++) {
      ...
      if (!king) {
        if (turn === "white") {
          if (
            !this.allPossibleMovesBlack(temp).includes(
              this.state.whiteKingPosition
            )
          ) {
          ...
          if (
            !this.allPossibleMovesWhite(temp).includes(
              this.state.blackKingPosition
            )
          ) {
            newList.push(highLightMoves[i]);
            ...
            <div className="game">
              ...
              <div className="game-info">
                ...
                <button
                  ...
                  style={{ display: this.state.hideButton }}
                >
                  {this.state.newGameButton}
                </button>
                <button
                  onClick={this.disconnect}
                  disabled={this.state.disableLeaveGameButton}
                  style={{ display: this.state.hideButton }}
                >
                  {this.state.leaveButton}
                </button>
                <button
                  onClick={this.resignButton}
                  style={{ display: this.state.hideResignButton }}
                >
                  Resign
                </button>
                <button
                  onClick={this.drawButton}
                  style={{ display: this.state.hideDrawButton }}
                >
                  Draw
                </button>
              </div>
